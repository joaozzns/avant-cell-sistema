...
--
-- A loja importa o extrato da maquininha (Cielo, Rede, Stone, PagSeguro…) e o
-- sistema procura, para cada lançamento, a venda correspondente. O que casa
-- vira 'matched'; o que casa mas com taxa diferente da combinada vira
-- 'divergent' — é assim que se descobre que a maquininha está cobrando mais do
-- que o contrato.
--
-- O casamento usa, em ordem: código de autorização, depois valor bruto com
-- data próxima (até 2 dias de diferença). Nunca casa o mesmo pagamento duas
-- vezes.
...
begin
  ...
  loop
    ...
    -- guarda o líquido real recebido no pagamento da venda
    update public.sale_payments
       set net_amount = v_s.net,
           fee_percent = case when v_s.gross > 0 then round(v_s.fee / v_s.gross * 100, 3) else fee_percent end
     where id = v_pag.id;
  end loop;
